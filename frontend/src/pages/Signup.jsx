...
const Signup = () => {
  ...
  // Keep the countries fetch just for a nice dropdown UI
  useEffect(() => {
    ...
    if (Object.keys(currentErrors).length === 0) {
      ...
      try {
        const response = await fetch(`${API_BASE}/api/auth/signup`, {
          ...
          credentials: 'omit' // We could use 'include' if CORS allows, but signup usually sets cookies internally by backend and standard frontend doesn't need to specify it unless dealing with cross-site cookies, wait actually 'include' is needed to RECEIVE the secure cookie! So 'include' it is.
        });
        ...
        // On success, backend returns { success: true, message: ..., data: { accessToken, user } }
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center flex-col items-center">
          ...
          <p className="mt-2 text-center text-sm text-slate-600">
            Register your company on Expensio
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-lg">
        <div className="bg-white py-8 px-4 shadow-xl shadow-slate-200/50 sm:rounded-2xl sm:px-10 border border-slate-100">
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    onBlur={handleBlur}
                    className={`block w-full appearance-none rounded-lg border px-3 py-2 placeholder-slate-400 shadow-sm focus:outline-none focus:ring-2 sm:text-sm ${errors.companyName ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : 'border-slate-300 focus:border-indigo-500 focus:ring-indigo-500'}`}
            ...
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-slate-700">Your Full Name</label>
              <div className="mt-1">
                <input
                  id="name"
                  name="name"
